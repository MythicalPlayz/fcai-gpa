...
export default function Term({ index, useNewBylaw }) {
  ...
  return <>
    ...
    <div className="flex flex-col justify-center items-center p-4 bg-blue-500 rounded-3xl text-white h-fit">
      <h4 className='font-bold'>Term {++index}</h4>
      ...
      <div className='flex flex-col justify-center items-center w-full'>
        ...
        {[...Array(courses)].map((_, course) => (
          <form key={course} className='flex flex-row my-2 justify-evenly items-center w-full space-x-2' action={""}>
            <input type="text" name={`name`} className='bg-white rounded-3xl text-black p-2 w-1/3' />
